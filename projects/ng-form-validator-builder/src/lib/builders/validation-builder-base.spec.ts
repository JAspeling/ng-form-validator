...
describe(`ValidationBuilderBase`, () => {
    ...
    beforeAll(() => { console.log('Executing [ValidationBuilder] spec'); });
    ...
    describe(`ifTrue`, () => {
        ...
        describe(`Not using the validationBuilderBase control value`, () => {
            it(`should add the validation error if the condition results in true`, () => {
                const result = validator.ifTrue(() => 50 < 100, 'error', `Expected to add an error when 50 < 100`).build();
                expect(result).toEqual({ error: 'Expected to add an error when 50 < 100' });
            });

            it(`should not add the validation error if the condition results in false`, () => {
                const result = validator.ifTrue(() => 50 > 100, 'error', `Shouldn't add an error, as (50 > 100) === false`).build();
        ...
        describe(`using the validationBuilderBase control value`, () => {
            ...
            it(`should add the validation error if the condition results in true (value === 'Johan')`, () => {
                const result = validator.ifTrue((value) => value === 'Johan', 'error', `Expected to add an error when value === 'Johan'`).build();
                expect(result).toEqual({ error: `Expected to add an error when value === 'Johan'` });
            });

            it(`should not add the validation error if the condition results in false (value === 'Hermanus')`, () => {
                const result = validator.ifTrue((value) => value === 'Hermanus', 'error', `Shouldn't add an error, as value ('Johan') !== 'Hermanus')`).build();
                expect(result).toEqual(null);
            });
        });
    });
    ...
    describe(`ifFalse`, () => {
        describe(`Not using the validationBuilderBase control value`, () => {
            it(`should add the validation error if the condition results in false`, () => {
                const result = validator.ifFalse(() => 50 > 100, 'error', 'Expected to add an error when the condition fails (50 > 100)').build();
                expect(result).toEqual({ error: 'Expected to add an error when the condition fails (50 > 100)' });
            });

            it(`should not add the validation error if the condition results in false`, () => {
                const result = validator.ifFalse(() => 50 < 100, 'error', `Shouldn't add an error, as the condition 50 < 100 passes, where it shouldn't`).build();
                expect(result).toEqual(null);
            });
        });
        ...
        describe(`using the validationBuilderBase control value`, () => {
            ...
            it(`should add the validation error if the condition results in false (value === 'Hermanus')`, () => {
                const result = validator.ifFalse((value) => value === 'Hermanus', 'error', `Expected to add an error, because value ('Johan') !== 'Hermanus'`).build();
                expect(result).toEqual({ error: `Expected to add an error, because value ('Johan') !== 'Hermanus'` });
            });

            it(`should not add the validation error if the condition results in false (value === 'Johan')`, () => {
                const result = validator.ifFalse((value) => value === 'Johan', 'error', `Shouldn't add an error, as the condition value ('Johan') === 'Johan'`).build();
                expect(result).toEqual(null);
            });
        });
    });
    ...
    describe(`withMessage`, () => {
        ...
        it(`should replace the previous errors with a custom message: {customError: '${OVERRIDE_ERROR}'}`, () => {
            ...
            const result = validator.withMessage(OVERRIDE_ERROR, 'customError').build();
        ...
        it(`should replace the previous errors with a custom message: {My error key with spaces: '${OVERRIDE_ERROR}'}`, () => {
            ...
            const result = validator.withMessage(OVERRIDE_ERROR, 'My error key with spaces').build();
